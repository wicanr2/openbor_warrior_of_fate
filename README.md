...
| [2026-07-24 runtime checkpoint](docs/RUNTIME_STATUS_2026-07-24.md) | fresh v2 disposable stage、39 個 Stage01 robot roster 替換、50 秒 Docker X11 visible run 與 attack macro 證據；明確列出 teardown 與 animation assertion 尚未關閉。 |
...
| [Getter 機械 hitflash closure](docs/GUANYU_GORE_REMAP.md) | `g1`、`g5`–`g11` 的 human blood hitflash 已改為既有 Electric model；含 exact-case 修正、private overlay 與 Docker smoke 證據。 |
...
| [Lidian runtime audit](research/LIDIAN_RUNTIME_AUDIT.md) | `lidian` 公開 keyposes、overview 圖與仍待完成的 production redraw / visible review 狀態。 |
...
| [Visible runner QA harness](scripts/run-openbor-visible-qa.sh) | 在有顯示環境的 runner 上錄 OpenBOR 視覺流程，並可用 `nu_select` / `nu_select_stage1` macro 驅動選角。 |
...
| [Boss family medium validator](scripts/validate-boss-family-medium.mjs) | 逐檔驗證六型 Boss family 的 405 張 GIF 是否符合 640×360 medium Boss canvas。 |
