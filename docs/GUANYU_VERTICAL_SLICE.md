...
> **美術方向淘汰警示（2026-07-15）：** 本頁記錄的 v1 紅／黑／金版本被確認更像「牛角武者鋼彈」，不符合關羽應採用的蓋特系方向。65 張 GIF、2 張 profile 與舊選角第一欄只能保留作 OpenBOR 畫布、Offset、palette 與 model-load 工程基準，不得再當角色定稿或交付藝術家描圖。v2 必須使用水平紅色頭部側翼、雙綠胸窗、紅色翼肩、銀白四肢與簡潔雙刃戰斧，並移除金色牛角、V-fin、武者兜及鋼彈式面罩。

![關羽蓋特系 v2 16 格 overview-only review image](../research/guanyu/guanyu-getter-v2-storyboard-v5-overview.png)

新 v2 canonical storyboard 以 v5 為準：單端雙刃戰斧在 idle、walk、guard、attack、pain 與 special 維持一致；frame 12 有兩腿兩腳與完整斧頭，frame 15 是可數出一頭、兩手、兩腿、兩腳的側躺姿勢，frame 16 是 exact parts inventory。公開總覽已用 edge-connected matte 正規化為 `#FC00FF`。65 主 GIF、2 profiles 與選角第一欄尚未由 v2 重建，因此下列 v1 runtime 結果仍只能作技術基準。
...
因此正確狀態是「關羽 v1 engineering overlay 已通過靜態與 model-load gate，但美術方向已淘汰」。在 v2 全量重繪、上述 deferred 全部關閉前，不得寫成「完整關羽玩家角色」、「production-ready」或「已完成玩法驗收」。
