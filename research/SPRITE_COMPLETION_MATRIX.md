...
|---|---|---:|---|---|---|
| 關羽／Getter／流龍馬 | `assets/players/guanyu` | 67 GIF | `getter-p1-interpolated-v1` 65 張獨立插值 raster；private `getter-v1` production-redraw seed 16/16 keyposes＋1 in-between seed；strict、palette、canvas gate | 其餘手繪清線／in-between；`g1`–`g16` 騎乘／武器；受擊、死亡、BBox／attack-box review | interpolated candidate＋complete keypose seed＋in-between seed |
