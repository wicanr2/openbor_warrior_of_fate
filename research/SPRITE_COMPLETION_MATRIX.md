...
| Stage01 機械前哨 | private `assets/environments/stage01-forest-outpost/candidates/stage01-original-visible-v2` | 4 GIF | 原創機械森林前哨 source 已切成可見 S2／panel／透明 foreground 三層；3/3 嚴格通過原 canvas、indexed palette、index-0 `#FC00FF`、無 GIF transparency extension。private `COMPOSITE-PREVIEW.png` 僅作交接檢視；public repo 不保存圖像。舊四層 P1 overlay 仍保留為 engineering reference。 | runtime parallax／viewport seam、foreground redraw、lighting 與 gameplay review | original visible background candidate＋strict manifest handoff |
