...
|---|---|---:|---|---|---|
| 關羽／Getter／流龍馬 | `assets/players/guanyu` | 67 GIF | `getter-p1-interpolated-v1` 65 張獨立插值 raster；另有 private `getter-v1` production-redraw seed 16/16 keyposes；strict、palette、canvas gate | 手繪清線／in-between；`g1`–`g16` 騎乘／武器；受擊、死亡、BBox／attack-box review | interpolated candidate＋complete keypose seed |
| 張飛／無敵鐵金剛／兜甲兒 | `assets/players/zhangfei` | 44 GIF | P0 41 physical GIF coverage；`mazinger-production-p1-interpolated-v1` 已產出 41 張獨立插值 raster 並通過 parity；private `mazinger-v1` production redraw seed 16/16 keyposes | 手繪清線／in-between、特殊技、抓投、倒地碎片、BBox review 與 2P palette | interpolated candidate＋complete keypose seed |
| 趙雲／EVA／碇真嗣 | `assets/players/zhaoyun` | 84 GIF | `eva-p1-interpolated-v1` 82 張獨立插值 raster；derived source；private `eva-v1` production redraw seed 16/16 keyposes | P1／P2 特效與抓投、手繪清線、武器／受擊逐格 review | derived interpolated candidate＋complete keypose seed |
...
| ν Gundam／第六角色／阿姆羅系 | `assets/players/nu-gundam` | 78 GIF＋2 TXT | `nu-p1-interpolated-v1` 73 張獨立插值 raster；Funnel proxy 保留；private `nu-v1` production redraw seed 13/16 keyposes | 3 個 keypose、Funnel orbit／beam／return／cleanup、手繪清線、P1 special、BBox review | interpolated candidate＋production seed |
