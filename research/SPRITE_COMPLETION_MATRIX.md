...
| 島田兵／藍盔一般敵 | `assets/enemies/blue-helmet-grunt/candidates/blue-helmet-p1-interpolated-v1` | 42 GIF | 42 張獨立插值 raster；42/42 strict parity、canvas、palette index 0 PASS；private `blue-helmet-v1` production redraw seed 9/12 keypose | 3 個 keypose、`shooter` 投射物／muzzle；`bingxs` 全部碎片；變體與攻擊姿勢逐格 production redraw | interpolated candidate＋production seed |
