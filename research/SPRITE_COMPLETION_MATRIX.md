...
| ν Gundam／第六角色／阿姆羅系 | `assets/players/nu-gundam` | 78 GIF＋2 TXT | `nu-p1-interpolated-v1` 73 張獨立插值 raster；private `funnel-v1` exact-canvas proxy candidates；private `nu-v1` production redraw seed 16/16 keyposes | Funnel orbit／beam／return 的獨立逐格 redraw、timing／cleanup、手繪清線、P1 special、BBox review | interpolated candidate＋complete keypose seed＋effect proxy |
