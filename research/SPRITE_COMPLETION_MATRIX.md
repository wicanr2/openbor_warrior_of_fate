...
| 島田兵／藍盔一般敵 | `assets/enemies/blue-helmet-grunt/candidates/blue-helmet-p1-interpolated-v1` | 42 GIF | 42 張獨立插值 raster；42/42 strict parity、canvas、palette index 0 PASS；private `blue-helmet-v1` 12/12 keypose＋11 full-chain in-between＋4 projectile＋3 death-fragment seed；另有 30 張 runtime GIF／BBox package；2026-07-20 保留畫面合成到鍵色後重索引，30/30 strict canvas／palette-index-0／no-transparency gate PASS | `offset/BBox／attack-box` 與逐格 production review | interpolated candidate＋runtime keypose/effect/in-between seeds |
...
| Stage01 機械前哨 | private `assets/environments/stage01-forest-outpost/candidates/stage01-original-visible-v3` | 4 GIF | 原創機械森林前哨已切成可見 S2／panel／左右機械根系 foreground 三層；中央角色跑道保留清楚，3/3 嚴格通過原 canvas、indexed palette、index-0 `#FC00FF`、無 GIF transparency extension。private `COMPOSITE-PREVIEW.png` 僅作交接檢視；public repo 不保存圖像。舊四層 P1 overlay 與 v2 保留為 engineering reference。 | runtime parallax／viewport seam、lighting 與 gameplay review | original visible background candidate＋strict manifest handoff |
| 機械補給箱 | private `assets/props/mechanical-supply-box/candidates/box-original-visible-v2` | 3 GIF | 原創機械膠囊 idle、開裂與殘骸三格已輸出；3/3 嚴格通過原 canvas、indexed palette、index-0 `#FC00FF` 與無 GIF transparency extension。private `CONTACT-PREVIEW.png` 用於交接檢視，public repo 不保存圖像；舊 P1 candidate 保留做 engineering reference。 | OpenBOR placement／掉落物可見性、金屬破壞音效與其他容器／道具 sprite | original visible prop candidate＋strict manifest handoff |
| 小型補給箱／`baoxiang2` | private `assets/props/mechanical-small-supply-box/candidates/baoxiang2-original-visible-v1` | 2 GIF | 原創完整／破損輕型補給箱依 `1.gif`、`2.GIF` 的 50×41 canvas 輸出；2/2 嚴格通過 indexed palette、index-0 `#FC00FF` 與無 GIF transparency extension。 | Boss 前 placement、掉落物可見性與 sound review | original visible prop candidate＋strict manifest handoff |
| 戰術能量信標／`firew` | private `assets/props/tactical-energy-beacon/candidates/firew-original-visible-v1` | 3 GIF | 原創 dormant／active column／overload 三格已按 `b1.GIF`–`b3.GIF` 48×73 canvas 輸出；3/3 嚴格通過 indexed palette、index-0 `#FC00FF` 與無 GIF transparency extension。private preview 只用於交接，public repo 不保存圖片。 | 對應 script 效果、關卡 placement 與音效 review | original visible prop candidate＋strict manifest handoff |
