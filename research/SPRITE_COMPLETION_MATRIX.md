...
|---|---|---:|---|---|---|
| 關羽／Getter／流龍馬 | `assets/players/guanyu` | 67 GIF | `getter-p1-interpolated-v1` 65 張獨立插值 raster；另有 private `getter-v1` production-redraw seed 8/16 keyposes；strict、palette、canvas gate | 8 個 keypose、手繪清線／in-between；`g1`–`g16` 騎乘／武器；受擊、死亡、BBox review | interpolated candidate＋production seed |
