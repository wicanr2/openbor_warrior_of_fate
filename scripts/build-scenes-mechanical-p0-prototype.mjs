...
const BASE = join(ROOT, '../workplace/extracted/data/scenes');
const OUT = join(ROOT, '../openbor_security_materal/assets/ui/scenes-mechanical-p0-v1');
function walk(root) {
  const result = [];
...
const temp = mkdtempSync(join(tmpdir(), 'scenes-p0-'));
const records = [];
try {
  for (let index = 0; index < files.length; index += 1) {
    ...
    const output = join(OUT, 'data/scenes', rel);
    mkdirSync(dirname(output), { recursive: true });
    let mode = 'blank-tiny-canvas';
    let paletteMode = null;
    if (image.width > 2 && image.height > 2) {
      const png = join(temp, `${index}.png`);
      run(['-hide_banner', '-loglevel', 'error', '-y', '-i', source, '-vf', `format=rgb24,colorchannelmixer=rr=0.62:gg=0.78:bb=1.05,drawbox=x=0:y=0:w=1:h=1:color=0xFC00FF:t=fill`, '-frames:v', '1', png]);
    ...
    records.push({ path: `data/scenes/${rel}`, canvas: [image.width, image.height], mode, palette: paletteMode });
    if ((index + 1) % 100 === 0) console.log(`scenes: ${index + 1}/${files.length}`);
  }
} finally { rmSync(temp, { recursive: true, force: true }); }
mkdirSync(OUT, { recursive: true });
writeFileSync(join(OUT, 'BUILD-MANIFEST.json'), `${JSON.stringify({ schemaVersion: 1, status: 'engineering-prototype-not-production-ready', productionReady: false, source: 'original scene silhouettes with mechanical blue palette remap', gifCount: records.length, deferred: ['redraw title/logo/story artwork', 'scene-specific typography and palette', 'dialogue portrait and cut-in review', 'runtime QA'], files: records }, null, 2)}\n`);
console.log(`Built ${records.length} scene GIFs at ${OUT}`);
